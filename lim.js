...
class NeuraBot {
  ...
  async performSwap(tokenIn, tokenOut, amountInStr) {
    ...
    try {
        ...
        logger.info(`Estimating gas for swap...`);

        // ESTIMATE GAS DYNAMICALLY to fix "Upfront cost exceeds balance"
        let estimatedGas;
        try {
            estimatedGas = await this.neuraWallet.estimateGas({
                to: CONTRACTS.NEURA.SWAP_ROUTER,
                data,
                value: txValue
            });
        } catch (gasError) {
            logger.warn(`Gas estimation failed, using fallback gas limit (350k).`);
        ...
        logger.info(`Sending swap transaction (Gas Limit: ${gasLimitWithBuffer})...`);
        ...
        // Try to extract JSON RPC error message if available
        if (e?.info?.error?.message) {
             msg = e.info.error.message;
        } else if (e.message.includes('Upfront cost')) {
            msg = "Insufficient ANKR balance to cover gas + value.";
        }
